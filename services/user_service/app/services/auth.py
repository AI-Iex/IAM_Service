...
class AuthService(IAuthService):
    ...
    def __init__(self,  uow_factory: UnitOfWorkFactory = async_unit_of_work, user_repo: IUserRepository = None, auth_repo: IAuthRepository = None):
    ...
    async def refresh_with_refresh_token(self, presented_raw: str, presented_jti: UUID | None,
        ...
        async with self.uow_factory() as db:
            ...
            token_row = None
            if presented_jti:
                token_row = await self.auth_repo.get_refresh_token_by_jti(db, presented_jti)
            else:
                presented_hashed = hash_refresh_token(presented_raw)
                token_row = await self.auth_repo.get_by_token_hash(db, presented_hashed)
            ...
            if token_row.revoked or token_expires < now:
                ...
                await self.auth_repo.revoke_all_refresh_tokens_for_user(db, token_row.user_id)
            ...
            await self.auth_repo.create_refresh_token(db, jti=new_jti, user_id=token_row.user_id, hashed_token=new_hashed, expires_at=new_expires_at, client_ip=ip, user_agent=user_agent)
            await self.auth_repo.mark_refresh_token_replaced(db, old_jti=token_row.jti, new_jti=new_jti)
            ...
            await self.auth_repo.update_refresh_token_last_used(db, jti=new_jti, used_at = datetime.now(timezone.utc))
    ...
    async def logout(self, user_id: UUID, jti: UUID):
        ...
        async with self.uow_factory() as db:
            ...
            token = await self.auth_repo.get_refresh_token_by_jti(db, jti)
            if not token:
            ...
            await self.auth_repo.revoke_refresh_token(db, jti)
    ...
    async def revoke_refresh_token_by_raw(self, raw_token: str):
        ...
        async with self.uow_factory() as db:
            ...
            token = await self.auth_repo.get_by_token_hash(db, hashed)
            if not token:
                logger.info("Refresh token given not found")
                return
            ...
            # 3. Revoke refresh token
            await self.auth_repo.revoke_refresh_token(db, token.jti)
    ...
    async def client_credentials(self, client_id: UUID, client_secret: str) -> dict:
        """Validate client_id/secret and issue an access token (no refresh token).

        Returns: {"tokens": TokenPair(...)}
        """
        async with self.uow_factory() as db:
            # use ClientRepository to find client
            client_repo = ClientRepository()
            client = await client_repo.get_by_client_id(db, client_id)
            if not client or not client.is_active:
                raise DomainError("Invalid client credentials")

            # verify secret using same password hasher
            if not verify_password(client_secret, client.hashed_secret):
                raise DomainError("Invalid client credentials")

            # issue access token with client subject and scopes
            scopes = (client.scopes or "").split(",") if client.scopes else []
            token_info = create_access_token(subject=str(client.client_id), roles=[], is_superuser=False)
            access_token = token_info.access_token
            expires_in = token_info.expires_in

            from app.schemas.auth import TokenPair
            return {"tokens": TokenPair(access_token=access_token, refresh_token=None, jti=token_info.get("jti"), expires_in=expires_in)}
