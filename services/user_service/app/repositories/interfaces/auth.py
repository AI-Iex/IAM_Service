...
class IAuthRepository(ABC):
    ...
    @abstractmethod
    async def create_refresh_token(self, db: AsyncSession, jti: UUID, user_id: UUID, hashed_token: str, expires_at: datetime, user_agent: Optional[str], client_ip: Optional[str]) -> RefreshToken:
    ...
    @abstractmethod
    async def update_refresh_token_last_used(self, db: AsyncSession, jti: UUID, used_at: Optional[datetime] = None) -> None:
