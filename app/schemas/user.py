...
class UserBase(BaseModel):
    ...
    full_name: str = Field(
        ..., json_schema_extra={"example": "Name Surname"}, description="The user's name and surname"
    )
    is_active: bool = Field(default=True, json_schema_extra={"example": True}, description="Is the user active?")
    is_superuser: bool = Field(
        default=False,
        json_schema_extra={"example": False},
        description="Is the user a superuser for get all permissions?",
    )
...
class UserRegister(BaseModel):
    ...
    password: str = Field(
        ..., json_schema_extra={"example": "strongpassword123"}, repr=False, description="The user's password"
    )
...
class UserCreateInDB(UserBase):
    ...
    require_password_change: bool = Field(
        default=False, json_schema_extra={"example": False}, description="Flag to force the user change the password"
    )
...
class UserUpdate(BaseModel):
    full_name: Optional[str] = Field(
        None, json_schema_extra={"example": "Name Surname"}, description="The user's name and surname"
    )
    is_active: Optional[bool] = Field(None, json_schema_extra={"example": True}, description="Is the user active?")
    is_superuser: Optional[bool] = Field(
        None, json_schema_extra={"example": False}, description="Is the user a superuser for get all permissions?"
    )
    roles: Optional[List[str]] = Field(
        None, json_schema_extra={"example": ["admin", "developer"]}, description="List of roles assigned to the user"
    )
...
class UserUpdateInDB(BaseModel):
    ...
    email: Optional[str] = Field(
        None, json_schema_extra={"example": "user@gmail.com"}, description="The user's email address"
    )
    full_name: Optional[str] = Field(
        None, json_schema_extra={"example": "Name Surname"}, description="The user's name and surname"
    )
    ...
    roles: Optional[List] = Field(
        None, description="Roles to assign (can be ORM role entities when passed from service)"
    )
...
class UserChangeEmail(BaseModel):
    current_email: str = Field(
        ...,
        json_schema_extra={"example": "currentemail@email.com"},
        description="Current email address for verification",
    )
    new_email: str = Field(
        ..., json_schema_extra={"example": "newemail@email.com"}, description="The new email address"
    )
    current_password: str = Field(
        ..., json_schema_extra={"example": "CurrentPassword123!"}, description="Current password for verification"
    )
...
class UserRead(UserBase):
    id: UUID = Field(
        ..., json_schema_extra={"example": "c55df1d2-216a-4359-81xx-1d805801vg0g"}, description="Unique user identifier"
    )
    ...
    updated_at: Optional[datetime] = Field(
        None,
        json_schema_extra={"example": "2023-01-05T15:30:00Z"},
        description="The date and time when the user was last updated",
    )
    last_login: Optional[datetime] = Field(
        None,
        json_schema_extra={"example": "2023-01-10T12:34:56Z"},
        description="The date and time of the user's last login",
    )
    roles: List[RoleReadWithoutPermissions] = Field(
        default=[], json_schema_extra={"example": ["admin", "user"]}, description="List of roles assigned to the user"
    )
    require_password_change: bool = Field(
        default=False, json_schema_extra={"example": False}, description="Flag to force the user change the password"
    )
...
class UserLogin(BaseModel):
    ...
    password: str = Field(
        ..., json_schema_extra={"example": "strongpassword123"}, description="The user's password", repr=False
    )
...
class PasswordChange(BaseModel):
    old_password: str = Field(
        ..., json_schema_extra={"example": "oldpassword123"}, description="The user's current password", repr=False
    )
