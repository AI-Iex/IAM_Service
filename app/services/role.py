...
class RoleService(IRoleService):
    ...
    async def assign_permission(self, role_id: UUID, permission_id: UUID) -> RoleRead:
        ...
        async with self._uow_factory() as db:
            ...
            # 5. Log the success
            logger.info(
                "Permission added to role successfully", extra={"role_id": role_id, "permission_id": permission_id}
            )
    ...
    async def remove_permission(self, role_id: UUID, permission_id: UUID) -> RoleRead:
        ...
        async with self._uow_factory() as db:
            ...
            # 5. Log the success
            logger.info("Permission removed from role", extra={"role_id": role_id, "permission_id": permission_id})
