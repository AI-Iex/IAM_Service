...
from app.schemas.health import HealthCheckResponse, DependencyHealth

logger = logging.getLogger(__name__)


class HealthService(IHealthService):
    def __init__(self, health_repo: IHealthRepository, uow_factory: UnitOfWorkFactory):
    ...
    async def check_readiness(self) -> HealthCheckResponse:
        """Perform readiness checks for service dependencies."""
        async with self._uow_factory() as db:
            ...
            return HealthCheckResponse(status=global_status, timestamp=datetime.utcnow(), checks=checks)
