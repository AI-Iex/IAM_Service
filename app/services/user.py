...
class UserService(IUserService):
    ...
    def _validate_password(self, password: str):
        ...
        if char_rules["special_chars"]["required"]:
            ...
            if len(re.findall(pattern, password)) < char_rules["special_chars"]["min_count"]:
                raise DomainError(f"Password must contain at least one special character ({allowed})")
    ...
    def _validate_name(self, name: str):
        ...
        if not special_allowed:
            for c in name:
                ...
                raise DomainError(
                    f"Name has this character '{c}' not allowed. Special chars allowed: '{allowed_special}'"
                )
    ...
    async def register_user(self, payload: UserRegister) -> UserRead:
        ...
        async with self._uow_factory() as db:
            ...
            # 7. Log the success
            logger.info("User registered successfully", extra={"user_created": user.id, "email": user.email})
    ...
    async def create(self, payload: UserCreateByAdmin) -> UserRead:
        ...
        async with self._uow_factory() as db:
            ...
            # 7. Log the success
            logger.info("User created successfully", extra={"user_created": user.id, "email": user.email})
    ...
    async def read_with_filters(
        ...
        async with self._uow_factory() as db:
            ...
            # 2. Log the success
            logger.info("Users retrieved successfully", extra={"Users read count": len(users)})
    ...
    async def change_email(self, user_id: UUID, payload: UserChangeEmail) -> UserRead:
        ...
        # 0. Log the attempt
        logger.info(
            "Requesting email change", extra={"current_email": payload.current_email, "new_email": payload.new_email}
        )
    ...
    async def assign_role(self, user_id: UUID, role_id: UUID) -> UserRead:
        ...
        async with self._uow_factory() as db:
            ...
            # 5. Log the success
            logger.info("Role added to user successfully", extra={"request_by_user_id": user_id, "role_id": role_id})
    ...
    async def remove_role(self, user_id: UUID, role_id: UUID) -> UserRead:
        ...
        async with self._uow_factory() as db:
            ...
            # 5. Log the success
            logger.info(
                "Role removed from user successfully", extra={"request_by_user_id": user_id, "role_id": role_id}
            )
    ...
    async def delete(self, user_id: UUID) -> None:
        ...
        async with self._uow_factory() as db:
            ...
            # 3. Log the success
            logger.info(
                "User deleted successfully", extra={"user_deleted": user_id, "user_email_deleted": existing_user.email}
            )
