...
class PermissionService(IPermissionService):
    ...
    async def create(self, payload: PermissionCreate) -> PermissionRead:
        ...
        async with self._uow_factory() as db:
            ...
            # 4. Log the success
            logger.info("Permission created successfully", extra={"permission_id": str(permission.id)})
    ...
    async def read_by_id(self, permission_id: UUID) -> PermissionRead:
        ...
        async with self._uow_factory() as db:
            ...
            # 3. Log the success
            logger.info("Permission read successfully", extra={"permission_found": permission.id})
    ...
    async def read_with_filters(
        ...
        logger.info(
            ...
            extra={"name_filter": name, "description_filter": description, "skip": skip, "limit": limit},
        )
    ...
    async def update(self, permission_id: UUID, payload: PermissionUpdate) -> PermissionRead:
        ...
        async with self._uow_factory() as db:
            ...
            if new_name != existing_permission.name:
                permission_with_name = await self._permission_repo.read_with_filters(db=db, name=[new_name], limit=1)
            ...
            update_payload = (
                PermissionUpdateInDB(**payload.model_dump(exclude_unset=True)) if payload else PermissionUpdateInDB()
            )
            updated_permission = await self._permission_repo.update(db, permission_id, update_payload)

            # 5. Log the success
            logger.info("Permission updated successfully", extra={"updated_permission_id": updated_permission.id})
    ...
    async def delete(self, permission_id: UUID) -> None:
        ...
        async with self._uow_factory() as db:
            ...
            # 4. Log the success
            logger.info("Permission deleted successfully", extra={"deleted_permission_id": permission_id})
