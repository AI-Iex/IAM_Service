...
async def test_read_user_by_id(async_client: AsyncClient, db_session, token_headers):
    ...
    headers = await token_headers(admin_email, admin_password)
    payload = {"email": f"uid_{uuid4().hex[:6]}@gmail.com", "full_name": "Target ReadById", "password": "StrongPass1!", "is_active": True, "is_superuser": False}
    create_resp = await async_client.post(f"{settings.route_prefix}/users", json = payload, headers = headers)
    assert create_resp.status_code == 201
...
async def test_change_email_and_password(async_client: AsyncClient, db_session, token_headers):
    ...
    create_payload = {"email": email, "full_name": "Self User", "password": old_password, "is_active": True, "is_superuser": False}
...
async def test_assign_and_remove_role(async_client: AsyncClient, db_session, token_headers):
    ...
    headers = await token_headers(admin_email, admin_password)
    create_payload = {"email": f"role_user_{uuid4().hex[:6]}@gmail.com", "full_name": "Role Target", "password": "StrongPass1!", "is_active": True, "is_superuser": False}
    create_resp = await async_client.post(f"{settings.route_prefix}/users", json=  create_payload, headers = headers)
    assert create_resp.status_code == 201
    created = create_resp.json()
    ...
    resp2 = await async_client.delete(f"{settings.route_prefix}/users/{created['id']}/roles/{role['id']}", headers = headers)
    assert resp2.status_code == 200
    data2 = resp2.json()
...
async def test_assign_and_remove_role_unauthenticated(async_client: AsyncClient, db_session, token_headers):
    ...
    create_payload = {"email": f"role_user_noauth_{uuid4().hex[:6]}@gmail.com", "full_name": "Role Target Noauth", "password": "StrongPass1!", "is_active": True, "is_superuser": False}
    create_resp = await async_client.post(f"{settings.route_prefix}/users", json = create_payload, headers = headers)
    assert create_resp.status_code == 201
    created = create_resp.json()
...
async def test_delete_user(async_client: AsyncClient, db_session, token_headers):
    ...
    headers = await token_headers(admin_email, admin_password)
    create_payload = {"email": f"del_{uuid4().hex[:6]}@gmail.com", "full_name": "To Delete", "password": "StrongPass1!", "is_active": True, "is_superuser": False}
    create_resp = await async_client.post(f"{settings.route_prefix}/users", json = create_payload, headers = headers)
    assert create_resp.status_code == 201
    created = create_resp.json()
...
async def test_delete_user_unauthenticated(async_client: AsyncClient, db_session, token_headers):
    ...
    create_payload = {"email": f"del_noauth_{uuid4().hex[:6]}@gmail.com", "full_name": "To Delete Noauth", "password": "StrongPass1!", "is_active": True, "is_superuser": False}
    create_resp = await async_client.post(f"{settings.route_prefix}/users", json = create_payload, headers = headers)
    assert create_resp.status_code == 201
    created = create_resp.json()
