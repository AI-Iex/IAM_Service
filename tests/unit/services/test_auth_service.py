...
async def test_login_success():
    ...
    svc = AuthService(uow_factory = lambda: DummyUoW(), user_repo = user_repo, refresh_token_repo = refresh_repo, client_repo = MagicMock(), auth_repo = auth_repo)
...
async def test_login_user_not_found_raises():
    ...
    svc = AuthService(uow_factory = lambda: DummyUoW(), user_repo = user_repo, refresh_token_repo = MagicMock(), client_repo = MagicMock(), auth_repo = MagicMock())
...
async def test_refresh_with_invalid_token_raises():
    ...
    svc = AuthService(uow_factory = lambda: DummyUoW(), user_repo = MagicMock(), refresh_token_repo = refresh_repo, client_repo = MagicMock(), auth_repo = MagicMock())
...
async def test_refresh_with_valid_token_rotates():
    ...
    token_row = SimpleNamespace(jti = jti, user_id = uuid4(), revoked = False, expires_at = future, hashed_token = hash_refresh_token(raw))
    ...
    svc = AuthService(uow_factory = lambda: DummyUoW(), user_repo = MagicMock(), refresh_token_repo = refresh_repo, client_repo = MagicMock(), auth_repo = auth_repo)
...
async def test_logout_token_not_found_raises():
    ...
    svc = AuthService(uow_factory = lambda: DummyUoW(), user_repo = user_repo, refresh_token_repo = refresh_repo, client_repo = MagicMock(), auth_repo = MagicMock())
...
async def test_client_credentials_success_and_failure():
    ...
    client = SimpleNamespace(id = uuid4(), client_id = uuid4(), is_active = True, hashed_secret = hash_password("secret123"), permissions = [])

    client_repo = MagicMock()
    client_repo.read_by_clientid = AsyncMock(return_value = client)

    svc = AuthService(uow_factory = lambda: DummyUoW(), user_repo = MagicMock(), refresh_token_repo = MagicMock(), client_repo = client_repo, auth_repo = MagicMock())
